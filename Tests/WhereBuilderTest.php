...
class WhereBuilderTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testSimpleExpression()
    {
        ...
        $this->assertEquals("WHERE `columnName` = ?", ltrim($builder->toString()));
    ...
    public function testMultiExpressions()
    {
        ...
        $this->assertEquals("WHERE `columnName` = ? AND `columnName2` = ?", ltrim($builder->toString()));
        $this->assertEquals("value", $builder->binds[0]);
    ...
    public function testExpressionWithTable()
    {
        ...
        $this->assertEquals("WHERE `table`.`columnName` = ?", ltrim($builder->toString()));
    }
    ...
    public function testLogicalOperatorExpression()
    {
        ...
        $this->assertEquals("WHERE `columnName` = ? OR `columnName2` = ?", ltrim($builder->toString()));
    }
    ...
    public function testComparatorExpression()
    {
        ...
        $this->assertEquals("WHERE `columnName` = ? AND `columnName2` < 10", ltrim($builder->toString()));
    }
    ...
    public function testGroupedExpressions()
    {
        ...
        $this->assertEquals("WHERE `columnName` = ? OR (`columnName2` < 10 AND `columnName3` = ?)", ltrim($builder->toString()));
    }
    ...
    public function testInExpression()
    {
        ...
        $this->assertEquals("WHERE `columnName` IN (?,10)", ltrim($builder->toString()));
        $this->assertEquals("value", $builder->binds[0]);
    }
}
